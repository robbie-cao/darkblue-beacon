...
# Check libusb-dev is installed on OSX & Linux
if [[ "$OSTYPE" == *darwin* ]]; then
echo "libusb-config version"
if ( ! ( libusb-config --version ) ); then echo "libusb-config not found! Ensure you have installed the libusb-compat package via macports.org"; exit 1 ; fi
fi
...
make CFLAGS="-L`pwd`/../libusb-win32-src-$LIBUSB_WIN32_VER/ -I`pwd`/../libusb-win32-src-$LIBUSB_WIN32_VER/src -I/opt/local/include $EXTRA_CFLAGS -lusb" install
cd ..
...
../openocd/configure $EXTRA_OPENOCD_CFGOPTS --enable-dummy --enable-ep93xx --enable-at91rm9200 --enable-usbprog --enable-jlink --enable-vsllink --enable-rlink --enable-arm-jtag-ew --enable-ft2232_libftdi --enable-usb_blaster_libftdi --enable-presto_libftdi --disable-option-checking  --prefix=`pwd`/../openocd-install/ --program-suffix=-all-brcm-libftdi --enable-maintainer-mode --enable-hndjtag --enable-static CFLAGS="-g -L`pwd`/../libftdi-install/lib/ -I`pwd`/../libftdi-install/include/ -L`pwd`/../libusb-win32-src-$LIBUSB_WIN32_VER/ -I`pwd`/../libusb-win32-src-$LIBUSB_WIN32_VER/src/  -I/opt/local/include -I`pwd`/../openocd/src/jtag/drivers/hndjtag/include/ $EXTRA_CFLAGS"
make
make install
cd ..
