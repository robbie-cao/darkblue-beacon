...
export CC_FOR_TARGET="$prefix/bin/$HOST_DIR/$target-gcc"
export AS="$prefix/bin/$HOST_DIR/$target-as"
export PATH=$prefix/bin/$HOST_DIR:$PATH

../$NEWLIB_SRC/configure --host=i686-pc-linux-gnu --target=$target --prefix=$prefix --enable-interwork --enable-multilib --disable-newlib-supplied-syscalls CFLAGS="-O3 -D_ELIX_LEVEL=0 -ffunction-sections -fdata-sections -B $prefix/bin/$HOST_DIR -I$prefix/lib/include -I$prefix/lib/include-fixed" || { echo "Error configuring newlib"; exit 1; }
