...
VOID WINAPI NotificationCallback(BTH_LE_GATT_EVENT_TYPE EventType, PVOID EventOutParameter, PVOID Context)
{
    ...
    ods ("EventType:%d\n", EventType);
    CWsSecOtaUpgradeDlg *pDlg = (CWsSecOtaUpgradeDlg *)Context;
...
BOOL CBtWin8Interface::Init()
{
    ...
    uuidWsUpgrade.Value.LongUuid = GUID_WS_SECURE_UPGRADE_SERVICE;
    for (int i = 0; i < num; i++)
    {
	...
	charControlPoint.Value.LongUuid = GUID_WS_SECURE_UPGRADE_CHARACTERISTIC_CONTROL_POINT;
    charData.IsShortUuid = FALSE;
	charData.Value.LongUuid = GUID_WS_SECURE_UPGRADE_CHARACTERISTIC_DATA;
    for (int i = 0; i < num; i++)
    {
...
void CBtWin8Interface::RegisterNotification()
{
    ...
    GUID guid = GUID_WS_SECURE_UPGRADE_SERVICE;
