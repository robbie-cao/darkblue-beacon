...
// WsSecOtaUpgrade.cpp : Defines the class behaviors for the application.
//
#include "stdafx.h"
#include "btwleapis.h"
...
BEGIN_MESSAGE_MAP(CWsSecOtaUpgradeApp, CWinApp)
	ON_COMMAND(ID_HELP, &CWinApp::OnHelp)
END_MESSAGE_MAP()


// CWsSecOtaUpgradeApp construction

CWsSecOtaUpgradeApp::CWsSecOtaUpgradeApp()
{
}


// The one and only CWsSecOtaUpgradeApp object

CWsSecOtaUpgradeApp theApp;
...
// CWsSecOtaUpgradeApp initialization

BOOL CWsSecOtaUpgradeApp::InitInstance()
{
	...
	CWsSecOtaUpgradeDlg dlg(pPatch, fileSize);
	...
	if (IsOSWin8())
	{
		...
		guidSvcWSUpgrade			  = GUID_WS_SECURE_UPGRADE_SERVICE;
		guidCharWSUpgradeControlPoint = GUID_WS_SECURE_UPGRADE_CHARACTERISTIC_CONTROL_POINT;
		guidCharWSUpgradeData         = GUID_WS_SECURE_UPGRADE_CHARACTERISTIC_DATA;
	}
	else if (IsOSWin7())
	{
		dlg.m_bWin8 = FALSE;
		...
		BtwGuidFromGuid(&guidSvcWSUpgrade, &GUID_WS_SECURE_UPGRADE_SERVICE);
        BtwGuidFromGuid(&guidCharWSUpgradeControlPoint, &GUID_WS_SECURE_UPGRADE_CHARACTERISTIC_CONTROL_POINT);
        BtwGuidFromGuid(&guidCharWSUpgradeData, &GUID_WS_SECURE_UPGRADE_CHARACTERISTIC_DATA);
	}
	else
	{
