...
#define LINE_COOK_FINE_TIMER           0
#define LINE_COOK_DEVICE_NAME          "line-cook"
#define LINE_COOK_DEVICE_APPEARENCE    0
#define LINE_COOK_MAIN_SERVICE_UUID    0
#define LINE_COOK_MAIN_CHAR_UUID       0
#define LINE_COOK_MAIN_CHAR_HANDLE     0
...
static void line_cook_create(void);
static void line_cook_connection_up( void );
static void line_cook_connection_down( void );
static void line_cook_advertisement_stopped( void );
static void line_cook_smp_bond_result( LESMP_PARING_RESULT result );
static void line_cook_encryption_changed( HCI_EVT_HDR *evt );
static int  line_cook_write_handler( LEGATTDB_ENTRY_HDR *p );
static void line_cook_interrupt_handler( UINT8 value );

/******************************************************
 *               Variables Definitions
 ******************************************************/

const BLE_PROFILE_CFG line_cook_cfg =
{
    /*.fine_timer_interval            =*/ LINE_COOK_FINE_TIMER, // ms
    ...
    /*.local_name                     =*/ LINE_COOK_DEVICE_NAME, // [LOCAL_NAME_LEN_MAX];
    /*.cod                            =*/ BIT16_TO_8(LINE_COOK_DEVICE_APPEARENCE),0x00, // [COD_LEN];
    ...
    /*.hdl                            =*/ {LINE_COOK_MAIN_CHAR_HANDLE, 0x00, 0x00, 0x00, 0x00}, // [HANDLE_NUM_MAX];
    /*.serv                           =*/ {LINE_COOK_MAIN_SERVICE_UUID, 0x00, 0x00, 0x00, 0x00},
    /*.cha                            =*/ {LINE_COOK_MAIN_CHAR_UUID, 0x00, 0x00, 0x00, 0x00},
...
const BLE_PROFILE_PUART_CFG line_cook_puart_cfg =
{
    /*.baudrate   =*/ 115200,
...
// NVRAM save area
HOSTINFO line_cook_hostinfo;
//pointer to the generated part of hostinfo assuming it is the beginning of the hostinfo
__HOSTINFO *p_hostinfo_generated = &line_cook_hostinfo.generated;

UINT16 	line_cook_connection_handle      = 0;                  // HCI handle of connection, not zero when connected
BD_ADDR line_cook_remote_addr            = {0, 0, 0, 0, 0, 0}; // Address of currently connected client
...
APPLICATION_INIT()
{
    bleapp_set_cfg((UINT8 *)gatt_database,
                   gatt_database_len,
                   (void *)&line_cook_cfg,
                   (void *)&line_cook_puart_cfg,
                   (void *)&line_cook_gpio_cfg,
                   line_cook_create);
}

// Create device
void line_cook_create(void)
{
    ...
    // Read NVRAM
    bleprofile_ReadNVRAM(VS_BLE_HOST_LIST, sizeof(line_cook_hostinfo), (UINT8 *)&line_cook_hostinfo);

    // register connection up and connection down handler.
    bleprofile_regAppEvtHandler(BLECM_APP_EVT_LINK_UP, line_cook_connection_up);
    bleprofile_regAppEvtHandler(BLECM_APP_EVT_LINK_DOWN, line_cook_connection_down);
    bleprofile_regAppEvtHandler(BLECM_APP_EVT_ADV_TIMEOUT, line_cook_advertisement_stopped);

    // handler for Encryption changed.
    blecm_regEncryptionChangedHandler(line_cook_encryption_changed);

    // handler for Bond result
    lesmp_regSMPResultCb((LESMP_SINGLE_PARAM_CB) line_cook_smp_bond_result);

    // register to process client writes
    legattdb_regWriteHandleCb((LEGATTDB_WRITE_CB)line_cook_write_handler);

    // register interrupt handler
    bleprofile_regIntCb((BLEPROFILE_SINGLE_PARAM_CB) line_cook_interrupt_handler);

    //registers timer
    line_cook_reg_timer();

    // advertise first vendor specific service
    if(sizeof(line_cook_uuid_main_vsc_service) == 16)
    {
		...
		memcpy(adv[1].data, &line_cook_uuid_main_vsc_service[0], 16);
...
void line_cook_connection_up(void)
{
	line_cook_connection_handle = (UINT16)emconinfo_getConnHandle();
	UINT8 *bda = (UINT8 *)emconninfo_getPeerPubAddr();

    // Save address of the connected device and print it out.
    memcpy(line_cook_remote_addr, bda, sizeof(line_cook_remote_addr));

    ble_trace3("connection_up: %08x%04x h=%d",
                (line_cook_remote_addr[5] << 24) + (line_cook_remote_addr[4] << 16) +
                (line_cook_remote_addr[3] << 8) + line_cook_remote_addr[2],
                (line_cook_remote_addr[1] << 8) + line_cook_remote_addr[0],
                line_cook_connection_handle);
...
void line_cook_connection_down(void)
{
    ble_trace1("connection_down:handle:%d", line_cook_connection_handle);

    line_cook_connection_handle = 0;
...
void line_cook_advertisement_stopped(void)
{
    ble_trace0("ADV stop!!!!");
...
void line_cook_smp_bond_result(LESMP_PARING_RESULT  result)
{
    ...
    if (result == LESMP_PAIRING_RESULT_BONDED)
    {
        ...
        line_cook_add_bond(bda);

        // ToDo: initialize persistent variables in HOSTINFO

        //now write hostinfo into NVRAM
        writtenbyte = bleprofile_WriteNVRAM(VS_BLE_HOST_LIST, sizeof(line_cook_hostinfo), (UINT8 *)&line_cook_hostinfo);
        ble_trace1("NVRAM write:%04x", writtenbyte);
    }
}

// Notification from the stack that encryption has been set.
void line_cook_encryption_changed(HCI_EVT_HDR *evt)
{
...
int line_cook_write_handler(LEGATTDB_ENTRY_HDR *p)
{
    UINT8  writtenbyte;
...
void line_cook_interrupt_handler(UINT8 value)
{
...
void line_cook_indication_cfm(void)
{
}
...
BOOL on_write_cooking_steps_cooking_step(int len, UINT8 *attrPtr)
{
...
// It will be called at the write handler and should return TRUE if any persistent value is changed
BOOL on_write_over_the_range_time(int len, UINT8 *attrPtr)
{
    //Todo: do you actions here when value is written by the peer
    // and return TRUE if any persistent value is changed
    return FALSE;
...
// It will be called at the write handler and should return TRUE if any persistent value is changed
BOOL on_write_over_the_range_fan(int len, UINT8 *attrPtr)
{
    //Todo: do you actions here when value is written by the peer
    // and return TRUE if any persistent value is changed
    return FALSE;
...
// It will be called at the write handler and should return TRUE if any persistent value is changed
BOOL on_write_over_the_range_light(int len, UINT8 *attrPtr)
{
    //Todo: do you actions here when value is written by the peer
    // and return TRUE if any persistent value is changed
    return FALSE;
